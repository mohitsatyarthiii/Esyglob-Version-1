...
export class AuthService {
  ...
  private async serializeUser(user: UserDocument): Promise<SerializedUser> {
    ...
    return {
      ...
      phone: user.phone,
      profileImage: user.profileImage ?? user.avatarUrl ?? user.avatar ?? user.image,
      avatar: user.avatar ?? user.avatarUrl ?? user.profileImage ?? user.image,
      image: user.image ?? user.profileImage ?? user.avatarUrl ?? user.avatar,
      roles: user.roles,
      activeRole,
