...
export function streamAgreementPdf(res, tradeDocument, entityNumber) {
  ...
  pdf.fillColor('#64748b').font('Helvetica').fontSize(8).text('International Commercial Agreement', 90, 63);
  pdf.y = 87;
  pdf.moveDown(.5).fillColor('#111827').fontSize(23).text(tradeDocument.title);
  pdf.moveDown(.25).fillColor('#64748b').font('Helvetica').fontSize(10).text(`Agreement ${content.agreementNumber || entityNumber}  |  Trade Reference ${entityNumber}  |  Version ${tradeDocument.version || content.revisionNumber || 1}`);
  pdf.moveDown(.2).fontSize(8.5).text(`Agreement Date: ${new Date(content.generatedAt || tradeDocument.createdAt).toLocaleDateString()}`);
  ...
  for (const signature of tradeDocument.signatures || []) {
    ...
    pdf.fillColor('#64748b').font('Helvetica').fontSize(8).text(`Signed ${new Date(signature.signedAt).toLocaleString()}  |  Method: ${heading(signature.signatureType)}  |  Agreement v${tradeDocument.version || 1}`);
  }
