...
import { AUTOMATED_PLATFORM_SERVICES } from './checkout-quote.js';
  ...
  const invoice = await Invoice.findOneAndUpdate(
    { orderId: order._id },
    {
      $setOnInsert: {
        ...
        issuedAt: new Date(),
        dueAt: order.estimatedDeliveryDate || order.estimatedDelivery || undefined,
      ...
      $set: {
        ...
        shipmentSnapshot: {
          ...
          estimatedDelivery: order.estimatedDeliveryDate || order.estimatedDelivery || null,
        },
      },
    },
...
export async function ensureOrderShipment(order, { status = 'pending', updatedBy } = {}) {
  ...
  const shipment = await Shipment.findOneAndUpdate(
    { orderId: order._id },
    {
      $setOnInsert: {
        ...
        insuranceStatus:
          Number(order.insuranceCost || selected.internalBreakdown?.insurance || 0) > 0
            ? 'included'
            : 'not_applicable',
      },
      $set: {
        ...
        estimatedDeliveryAt:
          order.estimatedDeliveryDate || order.estimatedDelivery || undefined,
  ...
  pushTimeline(
    order,
    status === 'pending' ? 'preparing_shipment' : status,
    event.description,
    updatedBy
  );
...
export async function markOrderPaymentSucceeded({ order, payment, updatedBy }) {
  ...
  order.paymentMethod =
    payment?.paymentMethod || payment?.method || order.paymentMethod || 'razorpay';
  ...
  pushTimeline(
    order,
    'confirmed',
    'Order confirmed and released to seller processing',
    updatedBy
  );
  ...
  pushTimeline(
    order,
    'invoice_generated',
    `Invoice ${invoice.invoiceNumber} generated automatically`,
    updatedBy
  );
  ...
  pushTimeline(
    order,
    'preparing_shipment',
    'Shipment record created and awaiting seller dispatch',
    updatedBy
  );

  order.platformServices = (order.platformServices || []).map((service) => {
    if (service.key === 'gst_invoice' || service.key === 'invoice_generation')
      return { ...service, status: 'issued' };
    if (service.key === 'shipment_tracking')
      return { ...service, status: 'preparing_shipment' };
...
export async function syncShipmentFromOrderStatus(
  order,
  { status, trackingNumber, estimatedDeliveryDate, updatedBy } = {}
) {
  ...
  const shipment = await ensureOrderShipment(order, {
  ...
  if (shipmentStatus) {
    ...
    shipment.events.push({
      ...
      description:
        ORDER_STATUS_LABELS[status] || `Shipment ${shipmentStatus}`,
      occurredAt: new Date(),
    });
  }
  ...
  if (shipment.estimatedDeliveryAt)
    order.estimatedDeliveryDate = shipment.estimatedDeliveryAt;
...
export function buildAutomatedOrderServices({
  quote,
  seller,
  product,
  logisticsOption,
