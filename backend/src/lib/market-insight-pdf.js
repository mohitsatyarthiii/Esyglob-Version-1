...
function cover(doc, report, metadata) {
  ...
  doc.fillColor('#8ea5c8').font('Helvetica').fontSize(7)
    ...
    .text(`Prepared by  EsyGlob Market Intelligence`, PAGE.left, 764)
...
function contents(doc, layout, report) {
  ...
  if (report.verificationNotice) {
    layout.ensure(70);
    const noticeY = doc.y;
    doc.roundedRect(PAGE.left, noticeY, PAGE.width, 58, 7).fillAndStroke('#fff8e8', '#e4bd68');
    doc.fillColor(COLORS.amber).font('Helvetica-Bold').fontSize(7.3).text('DECISION-USE NOTICE', PAGE.left + 12, noticeY + 10);
    doc.fillColor(COLORS.slate).font('Helvetica').fontSize(7).text(clean(report.verificationNotice), PAGE.left + 12, noticeY + 24, { width: PAGE.width - 24, height: 27, ellipsis: true, lineGap: 1.5 });
    doc.y = noticeY + 68;
    layout.mark('notice');
  }
}
...
function render(report, metadata, compact = false) {
  return new Promise((resolve, reject) => {
    ...
    for (let index = range.start; index < range.start + range.count; index += 1) {
      ...
      doc.text('CONFIDENTIAL • ESYGLOB MARKET INTELLIGENCE', PAGE.left, 800, { width: 245, lineBreak: false });
