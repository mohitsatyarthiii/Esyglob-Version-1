...
class MarketResearchService {
  ...
  static async run({ userId, session, query, productName = '', country = '', category = '', mode = 'product_rd' }, emit) {
    ...
    const globalTables = [
      ...(global.officialProductRows.length ? [{ title: `Verified product trade — HS ${global.hsCode}`, columns: ['rank', 'reporter', 'partner', 'hsCode', 'flow', 'period', 'valueUsd', 'source'], rows: global.officialProductRows }] : []),
      { title: 'Largest import markets — macro context', columns: ['rank', 'country', 'valueUsd', 'year', 'scope', 'source'], rows: global.macroImports.map(row => ({ ...row, valueUsd: fmtUsd(row.valueUsd) })) },
      { title: 'Largest export markets — macro context', columns: ['rank', 'country', 'valueUsd', 'year', 'scope', 'source'], rows: global.macroExports.map(row => ({ ...row, valueUsd: fmtUsd(row.valueUsd) })) },
      ...(global.publicArticles.length ? [{ title: 'Recent public market & industry sources', columns: ['title', 'domain', 'date', 'sourceType', 'url'], rows: global.publicArticles }] : []),
    ];
    const charts = [
    ...
    const sections = [
      { type: 'trade', title: 'Research scope & product classification', summary: global.hsCode ? `The supplied HS code ${global.hsCode} was used for the connected UN Comtrade query. Classification still requires destination-customs confirmation.` : 'No verifiable HS code was supplied. The report does not guess a classification, tariff, or product-level trade value.', points: generated.insights || [], confidence: global.hsCode ? 82 : 65, evidenceType: 'official-data-and-analysis' },
      { type: 'trade', title: 'Global trade intelligence', summary: 'Country rankings below use official aggregate imports and exports of goods and services as macroeconomic context. They are not represented as product market size.', points: [`${global.macroImports.length} import markets and ${global.macroExports.length} export markets were compared.`, global.target ? `Target-country indicators were matched for ${global.target.name}.` : 'No target-country record was selected.'], confidence: 92, evidenceType: 'official-data' },
