...
export async function previewDocument(req, res) {
  try {
    ...
    const signatures = ['seller', 'buyer'].map(role => { const item = (document.signatures || []).find(value => value.signerRole === role); return item ? `<article class="signature signed"><strong>${escapeHtml(role)} signature</strong><b>${escapeHtml(item.signerName)}</b><em>${escapeHtml(item.signatureType === 'typed' ? item.signatureValue : 'Drawn signature retained')}</em><small>Signed ${escapeHtml(new Date(item.signedAt).toLocaleString())} · Agreement v${escapeHtml(document.version || 1)}</small></article>` : `<article class="signature pending">Awaiting ${escapeHtml(role)} signature</article>`; }).join('');
