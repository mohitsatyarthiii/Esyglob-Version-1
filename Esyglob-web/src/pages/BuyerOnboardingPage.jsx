...
export default function BuyerOnboardingPage() {
  ...
  const validation = useMemo(() => {
    ...
    if (step === 3 && ([form.country, form.state, form.city, form.address].some(value => !String(value).trim()) || !/^[A-Z]{2}$/.test(String(form.countryCode || '').toUpperCase()))) return 'Select a complete delivery address with a valid country code.'
    return ''
  }, [form, step])
  ...
  return <AppShell><div className="container onboarding-page">
    ...
    <section className="module-panel onboarding-card"><header><Icon /><div><span>Step {step + 1} of 4</span><h2>{title}</h2></div></header>
      ...
      {step === 3 && <div className="form-grid"><label className="field-wide">Find delivery address<AddressAutocomplete value={form.address} onChange={address => setForm(current => ({ ...current, address }))} onSelect={location => setForm(current => ({ ...current, address: location.line1 || location.formattedAddress, country: location.country, countryCode: String(location.countryCode || '').toUpperCase(), state: location.state, city: location.city, district: location.district, postalCode: location.postalCode, placeId: location.placeId, latitude: location.latitude, longitude: location.longitude }))} /></label><Field label="Country" name="country" form={form} setForm={setForm} required /><Field label="Country code" name="countryCode" form={form} setForm={setForm} required /><Field label="State" name="state" form={form} setForm={setForm} required /><Field label="City" name="city" form={form} setForm={setForm} required /><Field label="Postal code (optional)" name="postalCode" form={form} setForm={setForm} /></div>}
