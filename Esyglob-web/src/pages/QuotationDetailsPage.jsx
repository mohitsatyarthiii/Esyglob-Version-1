...
const openStatuses = ['pending', 'submitted', 'negotiating', 'countered', 'revision_requested', 'revised']
export default function QuotationDetailsPage() {
  ...
  async function buyerAction(action) {
    ...
    try {
      let result
      if (action === 'accept' || action === 'reject') result = await respondToQuotation(quotationId, action, action === 'reject' ? { reason: actionText || 'Buyer rejected this quotation.' } : {})
      ...
      setDialog(''); setActionText(''); setCounterPrice(''); setMessage(action === 'accept' ? 'Quotation accepted and trade order created.' : 'Your response was sent to the supplier.'); await query.reload()
      const orderId = resolveId(result?.tradeOrder || result?.order || result?.orderId)
      if (action === 'accept' && orderId) navigate(`/orders/${orderId}`)
