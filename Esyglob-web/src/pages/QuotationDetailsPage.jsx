...
export default function QuotationDetailsPage() {
  ...
  useEffect(() => { let socket; const refresh = event => { if (!event?.quotationId || resolveId(event.quotationId) === quotationId) reloadQuotation() }; getRealtimeClient().then(client => { socket = client; client.on('quotation_updated', refresh) }).catch(() => {}); return () => socket?.off('quotation_updated', refresh) }, [reloadQuotation, quotationId])
  ...
  const item = sellerView && rawItem.directOrderEnabled && rawItem.status === 'final_quotation_signed'
    ? { ...rawItem, status: 'direct_order_waiting' }
    : rawItem
  ...
  async function buyerAction(action) {
    ...
    try {
      if (action === 'accept' || action === 'reject') await respondToQuotation(quotationId, action, action === 'reject' ? { reason: actionText || 'Buyer rejected this quotation.' } : {})
      else await updateQuotation(quotationId, { action, reason: actionText || (action === 'counter_offer' ? 'Buyer sent a counter offer.' : 'Buyer requested a revision.'), ...(action === 'counter_offer' && counterPrice ? { unitPrice: Number(counterPrice) } : {}) })
      setDialog(''); setActionText(''); setCounterPrice(''); setMessage(action === 'accept' ? 'Quotation accepted. The manufacturer will now prepare the Final Quotation.' : 'Your response was sent to the manufacturer.'); await query.reload()
    } catch (nextError) { setError(nextError.message) }
    finally { setBusy(false) }
  }
  async function sellerAction(action) {
    if (action === 'confirm') {
      document.getElementById('final-quotation-title')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
      return
    }
    setBusy(true); setError('')
    try { await updateQuotation(quotationId, { action, reason: actionText || undefined }); setMessage(action === 'withdraw' ? 'Quotation withdrawn.' : action === 'confirm' ? 'Final Quotation sent for Buyer signature.' : 'Quotation sent to buyer.'); await query.reload() }
    catch (nextError) { setError(nextError.message) }
    finally { setBusy(false) }
  }
