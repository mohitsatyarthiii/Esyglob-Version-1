...
export default function WalletPage() {
  ...
  const activity = useMemo(() => {
    ...
    const serviceById = new Map(
      services.map((item) => [resolveId(item), item])
    );
    const paymentEntityIds = new Set(
      (data.payments || [])
        .map((item) => resolveId(item.entityId))
        .filter(Boolean)
    );
    const servicePayments = services
      .filter(
        (item) =>
          ...
          !paymentEntityIds.has(resolveId(item))
      )
      .map((item) => ({
    ...
    return [
      ...
      ...(data.payments || []).map((item) => {
        ...
        return {
          ...
          section: "payment",
          direction: "debit",
          providerKey: service?.provider?.key,
        };
      }),
      ...servicePayments,
    ]
      ...
      .filter(
        (item) =>
          !search ||
          [item.description, item.type, item.reference, item.status].some(
            (value) =>
              String(value || "")
                .toLowerCase()
      ...
      if (action === "remove") {
        if (
          !(await confirm({
            ...
            message:
              "This saved account will no longer be available for future payments or withdrawals.",
    ...
    <AppShell>
      <div className="container module-page">
        <PageHead
          ...
          title="Wallet"
          description="Secure payout accounts, balances, credits, refunds and withdrawals in one ledger."
        />
          ...
          <>
            <section className="wallet-hero">
              <div>
                ...
                <b>
                  <Money
                    value={summary.balance || data.wallet?.balance || 0}
                    currency={data.wallet?.currency}
                  />
                </b>
                <p>
                  {role === "seller"
                    ? "Seller earnings and available settlements"
                    : "Buyer credits, refunds and payments"}
                </p>
              </div>
              <div>
                <Metric label="Credits" value={summary.totalCredits} />
                <Metric label="Refunds" value={summary.refundedAmount} />
                <Metric label="Escrow" value={summary.escrowBalance} />
                <Metric
                  label="Withdrawable"
                  value={summary.withdrawableAmount}
                />
              </div>
            </section>
            <div className="wallet-metrics">
              ...
              <Metric
                ...
                value={summary.orderPaymentTotal}
                money
              />
              <Metric
                ...
                money
              />
              <Metric label="Total debits" value={summary.totalDebits} money />
            </div>
            <section className="module-panel payment-methods-panel">
              <div className="compact-heading">
                <div>
                  <h2>Saved payment accounts</h2>
                  <p>
                    Bank and UPI accounts are usable only after RazorpayX
                    verification.
                  </p>
                </div>
                <ShieldCheck />
              ...
              {methods.length ? (
                <div className="payment-method-grid">
                  {methods.map((method) => {
                    ...
                    const label =
                      method.label ||
                      method.bankName ||
                      (method.type === "upi" ? "UPI account" : "Bank account");
                    return (
                      <article className="payment-method-card" key={id}>
                        <i>
                          {method.type === "upi" ? (
                            <Smartphone />
                          ) : (
                        ...
                        <div className="payment-method-copy">
                          ...
                          <p>
                            {method.type === "upi"
                              ? method.upiId
                              : `${method.maskedAccountNumber || "Account"} · ${
                                  method.ifsc || ""
                                }`}
                          </p>
                          <small>
                            {method.verificationMessage ||
                              "Awaiting verification"}
                          </small>
                        </div>
                        <StatusBadge
                          status={method.verificationStatus || "pending"}
                        />
                        <div className="payment-method-actions">
                          {method.verificationStatus !== "verified" && (
                            <button
                              ...
                              onClick={() => manage(method, "verify")}
                            >
                              <RefreshCw />{" "}
                              {methodBusy === `${id}:verify`
                                ? "Checking…"
                                : "Verify"}
                            </button>
                          )}
                          {method.verificationStatus === "verified" &&
                            !method.isDefault && (
                              <button
                                className="button button--secondary"
                                disabled={Boolean(methodBusy)}
                                onClick={() => manage(method, "set_default")}
                              >
                                <Star /> Make default
                              </button>
                            )}
                ...
                <div className="empty-results">
                  <Landmark />
                  <h2>No payment accounts</h2>
                  <p>
                    Add a bank account or UPI ID. It stays pending until the
              ...
              {activity.length ? (
                <div className="wallet-ledger">
                  {activity.map((item, index) => (
                    <article key={resolveId(item) || index}>
                      <i
                        className={
                      ...
                      <div>
                        <b>
                          {item.description ||
                            String(item.type || item.section).replaceAll(
                              "_",
                              " "
                            )}
                        </b>
                      ...
                      <StatusBadge
                        status={
                          item.status || item.paymentStatus || "completed"
                        }
                      />
                      <strong
                        className={
                      ...
                      >
                        {item.direction === "credit" ? "+" : "−"}
                        <Money
                          value={item.amount || item.totalAmount}
                ...
                <div className="empty-results">
                  <WalletCards />
                  <h2>No wallet activity</h2>
                  <p>Payments, credits and refunds will appear here.</p>
                </div>
              )}
            </section>
          </>
    ...
    <div className="modal-backdrop" onMouseDown={onClose}>
      ...
      >
        <div className="compact-heading">
          <div>
            <h2>
              {type === "method" ? "Add payment account" : "Withdraw funds"}
            </h2>
            <p>
              {type === "method" &&
                "Details are encrypted before storage and verified through RazorpayX."}
            </p>
          </div>
          ...
          <>
            ...
            <label>
              ...
              <input
                ...
                placeholder="For example: Primary settlement"
              />
            </label>
            ...
            <label className="check-field">
              ...
              />{" "}
              Make default after successful verification
            </label>
          </>
        ) : (
          <>
            <p>
              ...
              <b>
                <Money value={summary.withdrawableAmount || 0} />
              </b>
            </p>
            <label>
              ...
              >
                ...
                {verified.map((item) => (
                  <option value={resolveId(item)} key={resolveId(item)}>
                    {item.label ||
                      item.bankName ||
                      item.upiId ||
            ...
            {!verified.length && (
              <p className="action-error">
                Verify a bank or UPI account before requesting a withdrawal.
              </p>
            )}
            <label>
              Amount
        ...
        <button
          ...
          disabled={busy || (type === "withdraw" && !verified.length)}
        >
          {busy ? (
            "Saving…"
          ) : type === "method" ? (
            <>
              <Landmark /> Save and verify
            </>
          ) : (
            <>
              <CreditCard /> Request withdrawal
            </>
          )}
        </button>
      </form>
