    ...
    <aside className={sidebarOpen ? 'open' : ''} aria-label="Recent AI chats" aria-modal={sidebarOpen ? 'true' : undefined}>
      ...
      <div className="ai-history-list">{historyLoading ? <div className="ai-history-empty"><div className="typing-dots"><span /><span /><span /></div><p>Loading chats...</p></div> : visibleChats.length ? visibleChats.map((item) => {
        ...
        return <div className={`ai-history-row ${id === chatId ? 'active' : ''}`} key={id}>
          {editingId === id ? <form onSubmit={(event) => { event.preventDefault(); renameConversation(id) }}><input autoFocus maxLength={90} value={editingTitle} onChange={(event) => setEditingTitle(event.target.value)} onKeyDown={(event) => { if (event.key === 'Escape') setEditingId('') }} /><button title="Save name">Save</button></form> : <button className="ai-history-main" title={item.title || 'Marketplace assistant'} onClick={() => { if (!busy) { setChatId(id); setSidebarOpen(false); setError('') } }}><span><b>{item.title || 'Marketplace assistant'}</b><small>{timestamp ? formatChatTimestamp(timestamp) : `${item.totalMessages || 0} messages`}</small></span></button>}
    ...
    <section className={`ai-chat ${!messages.length && !conversationLoading ? 'ai-chat--empty' : ''}`}>
      <header>
        ...
        <div className="ai-header-actions">
          ...
          <button className="ai-header-tool ai-share-button" type="button" onClick={shareConversation} aria-label="Share conversation" title="Share conversation"><Share2 /></button>
