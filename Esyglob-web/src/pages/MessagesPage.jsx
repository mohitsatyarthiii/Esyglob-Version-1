...
export default function MessagesPage() {
  ...
  const query = useAsyncData(
    useCallback(
      () =>
        fetchChats({
          role,
          view: view === "groups" ? "all" : view,
          unreadOnly,
        }),
      [role, unreadOnly, view]
    )
  );
  ...
  const rows = useMemo(
    () =>
      (query.data || [])
        .filter((chat) => view !== "groups" || chat.chatType === "group")
        .filter(
          (chat) =>
            !search ||
            [
              chat.groupName,
              chat.lastMessage,
              displayName(chat.buyerId),
              displayName(chat.sellerId),
              chat.productId?.name,
            ].some((value) =>
              String(value || "")
                .toLowerCase()
  ...
  const unreadTotal = useMemo(
    () => (query.data || []).reduce((sum, chat) => sum + roleUnread(chat), 0),
    [query.data, roleUnread]
  );
  const contacts = useMemo(() => {
    ...
    (query.data || []).forEach((chat) => {
      ...
      if (id && id !== currentId)
        found.set(id, {
          id,
          name: displayName(other, "Marketplace contact"),
  ...
  useEffect(() => {
    ...
    const onPresence = (event) => {
      if (live && event.userId)
        setPresence((current) => ({
          ...current,
          [event.userId]: Boolean(event.online),
        }));
    };
    getRealtimeClient()
      .then((client) => {
        ...
        contacts.forEach((contact) =>
          client.emit("get_presence", { userId: contact.id }, (result) => {
            if (live)
              setPresence((current) => ({
                ...current,
                [contact.id]: Boolean(result?.online),
              }));
          })
        );
      })
  ...
  async function action(id, name, value) {
    try {
      await chatAction(id, name, value);
      setMenuId("");
      query.reload();
    } catch (error) {
  ...
  return (
    <AppShell>
      <div className="listing-page container trade-page messages-page">
        <div className="messages-title-row">
          <PageHead
            eyebrow="Buyer-seller messaging"
            title={unreadTotal ? `Messages (${unreadTotal})` : "Messages"}
            description="Product, RFQ and quotation conversations stay connected to their marketplace context."
          />
          <button
            className="message-new-group"
            onClick={() => {
              setGroupError("");
              setGroupOpen(true);
            }}
        ...
        <div className="trade-page-actions">
          <label className="unread-toggle">
            <input
              type="checkbox"
              checked={unreadOnly}
              onChange={(event) => set("unread", String(event.target.checked))}
            />{" "}
            Unread only
          </label>
        </div>
        <div className="messenger-tabs">
          {["all", "groups", "favorites", "archived"].map((item) => (
            <button
              className={view === item ? "active" : ""}
              key={item}
              onClick={() => set("view", item)}
            >
              {item === "all" ? "Chats" : item}
            </button>
          ))}
        ...
        {query.loading ? (
          ...
          <div className="conversation-list">
            {rows.map((chat) => {
              ...
              const participant =
                chat.chatType === "group"
                  ? chat.groupName
                  : displayName(other, chat.productId?.name || "Conversation");
              const unread = roleUnread(chat);
              const image = chat.chatType === "group" ? "" : avatarUrl(other);
              const online =
                chat.chatType !== "group" && presence[resolveId(other)];
              return (
                <Link
                  to={`/messages/${id}`}
                  key={id}
                ...
                >
                  <div className="conversation-avatar">
                    {image ? (
                      ...
                      String(participant).slice(0, 1).toUpperCase()
                    )}
                    <span
                      className={
                        online ? "presence-dot online" : "presence-dot"
                      }
                    />
                  </div>
                  <div className="conversation-copy">
                    ...
                    <span>
                      {chat.rfqId
                        ...
                        ? "Product inquiry"
                        : chat.chatType === "group"
                        ? `${chat.groupMembers?.length || 0} members`
                        : online
                        ? "Online"
                  ...
                  <button
                    ...
                    aria-label={`Actions for ${participant}`}
                    onClick={(event) => {
                      event.preventDefault();
          ...
          <div className="empty-results">
            ...
            <p>
              {view === "groups"
                ? "Create a group with people you already chat with."
                : "Start a contextual chat from a product, supplier, RFQ or quotation."}
            </p>
            {view === "groups" && (
              <button
        ...
        {groupOpen && (
          <GroupDialog
            ...
            error={groupError}
            close={() => {
              if (!creating) setGroupOpen(false);
            }}
            submit={createGroup}
          />
        )}
...
function ChatMenu({ chat, unread, close, action }) {
  ...
  const items = [
    ...
    [
      Heart,
      chat.isFavorite ? "Remove favorite" : "Favorite chat",
      "favorite",
      !chat.isFavorite,
    ],
    [
  ...
  return (
    <div className="modal-backdrop chat-menu-backdrop" onMouseDown={close}>
      <div
        className="chat-action-menu"
        role="menu"
        onMouseDown={(event) => event.stopPropagation()}
      >
        {items.map(([Icon, label, name, value]) => (
          <button key={name} onClick={() => action(id, name, value)}>
            <Icon />
            {label}
          </button>
        ))}
  ...
  return (
    <div className="modal-backdrop" onMouseDown={close}>
      ...
      >
        ...
        <div className="group-contact-list">
          ...
          {contacts.length ? (
            contacts.map((contact) => {
              const selected = members.includes(contact.id);
              return (
                <button
                  type="button"
            ...
            <p>
              Start individual chats first; those contacts will be available
              here.
            </p>
          )}
        </div>
        ...
        <button
          className="button button--primary"
          disabled={creating || groupName.trim().length < 2 || !members.length}
        >
          {creating
            ? "Creating…"
            : `Create Group${members.length ? ` (${members.length})` : ""}`}
        </button>
      </form>
    </div>
  );
