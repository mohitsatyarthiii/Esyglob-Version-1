import { BarChart3, Bell, Bot, Boxes, CheckCircle2, ChevronRight, CircleUserRound, CreditCard, FileText, Heart, HelpCircle, LockKeyhole, LogOut, MapPin, MessageSquare, PackageCheck, Plus, Settings, ShieldCheck, Store, Target, Truck, WalletCards } from 'lucide-react'
import { useCallback, useMemo, useState } from 'react'
import { Link, useNavigate } from 'react-router-dom'
import { fetchAccountSummary } from '../api/marketplace'
import { fetchChats, fetchNotifications, fetchQuotations, fetchRfqs } from '../api/trade'
...
const buyerSections = [
  ...
  { title: 'Buying Services', items: [
    ['My RFQs', '/rfqs', Target, 'amber', 'rfqs'], ['Quotations', '/quotations', FileText, 'green', 'quotations'], ['Wallet', '/wallet', WalletCards, 'green'], ['Saved Items', '/saved', Heart, 'rose'], ['Addresses', '/addresses', MapPin, 'sky'], ['Notifications', '/notifications', Bell, 'rose', 'notifications'],
  ] },
...
const sellerSections = [
  ...
  { title: 'Sales Services', items: [
    ['RFQs', '/rfqs?role=seller', Target, 'amber', 'rfqs'], ['Quotations', '/quotations?role=seller', FileText, 'green', 'quotations'], ['Wallet', '/wallet?role=seller', WalletCards, 'green'], ['Verification', '/seller/verification', ShieldCheck, 'sky'], ['Factory', '/seller/factory', Store, 'green'], ['Membership', '/subscriptions', CreditCard, 'violet'], ['Notifications', '/notifications', Bell, 'rose', 'notifications'],
  ] },
...
export default function AccountPage() {
  ...
  const query = useAsyncData(useCallback(async () => {
    const [summary, notifications, rfqs, quotations, chats] = await Promise.all([
      fetchAccountSummary(role), fetchNotifications({ limit: 1 }), fetchRfqs({ scope: role, limit: 1 }), fetchQuotations({ scope: role, limit: 1 }), fetchChats({ role, limit: 60 }),
    ])
    return { summary, notifications, rfqs, quotations, chats }
  }, [role]))
  ...
  const badges = useMemo(() => ({
    notifications: query.data?.notifications?.unreadCount || 0,
