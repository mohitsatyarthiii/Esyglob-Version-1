...
export default function ServiceBookingPage() {
  ...
  const providerFingerprint = useMemo(() => {
    ...
    const visibleRequired = service.fields.filter(
      (field) => field.required && isServiceFieldVisible(field, values)
    );
    if (
      visibleRequired.some((field) => !String(values[field.key] || "").trim())
    )
      return "";
    if (values.pickupEmail && !validEmail(values.pickupEmail)) return "";
    if (values.destinationEmail && !validEmail(values.destinationEmail))
      return "";
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setValues((formValues) => ({
        ...
        contactEmail:
          formValues.contactEmail || profile.email || user?.email || "",
        contactPhone:
          formValues.contactPhone || profile.phone || user?.phone || "",
        pickupContactName:
          formValues.pickupContactName || address.fullName || fullName,
        pickupPhone:
          formValues.pickupPhone ||
          address.phone ||
          profile.phone ||
          user?.phone ||
          "",
        pickupEmail:
          formValues.pickupEmail || profile.email || user?.email || "",
        pickupLine1:
          formValues.pickupLine1 ||
          address.address ||
          address.line1 ||
          profile.address ||
          "",
        ...
        pickupPostalCode:
          ...
          "",
        pickupCountry:
          address.country || formValues.pickupCountry || profile.country || "",
  ...
  async function loadProviders() {
    ...
    setProviderResult({
      providers: [],
      providerStatuses: LIVE_SHIPPING_PROVIDERS.map((provider) => ({
        provider,
        status: "checking",
      })),
    ...
    const results = await Promise.allSettled(
      LIVE_SHIPPING_PROVIDERS.map(async (providerKey) => {
        try {
          const result = await searchSingleServiceProvider(
            service.key,
            providerKey,
            input
          );
          if (sequence === providerRequest.current)
            setProviderResult((current) =>
              mergeProviderResult(current, result, providerKey)
            );
          return result;
        } catch (next) {
          if (sequence === providerRequest.current) {
            setProviderResult((current) =>
              mergeProviderFailure(current, providerKey, next.code)
            );
            if (next.fieldErrors && Object.keys(next.fieldErrors).length)
              setFieldErrors((current) => ({
                ...current,
                ...providerFieldErrors(next.fieldErrors),
              }));
          }
          throw next;
        }
    ...
    const usableRates = results.flatMap((result) =>
      result.status === "fulfilled" ? result.value.providers || [] : []
    );
    const allFailed =
      !usableRates.length &&
    ...
    if (allFailed)
      setError(
        "No shipping provider could return rates right now. Check the shipment details and try again."
      );
  }
  ...
  if (!isServiceAvailable(service))
    return (
      <AppShell>
        <div className="container module-page">
          <div className="marketplace-empty marketplace-empty--soon">
            <i>
              <Sparkles />
            </i>
            <span>Coming Soon</span>
            <h2>{service.title} is not open for booking yet</h2>
            <p>
              We are completing the provider workflow before accepting requests.
            </p>
            <Link
  ...
  if (service.role === "seller" && user?.primaryRole !== "seller")
    return (
      <AppShell>
        <div className="container module-page">
  ...
  function validate() {
    const errors = {};
    service.fields
      .filter((field) => isServiceFieldVisible(field, values))
      .forEach((field) => {
        const value = String(values[field.key] || "").trim();
        if (field.required && !value)
          errors[field.key] = `${field.label} is required.`;
        else if (field.type === "email" && value && !validEmail(value))
  ...
  function applyAddress(fieldKey, location) {
    const prefix = fieldKey.startsWith("pickup")
      ? "pickup"
      : fieldKey.startsWith("destination")
      ? "destination"
      : "";
    if (!prefix) {
    ...
    setValues((current) => ({
      ...
      [`${prefix}PostalCode`]:
        location.postalCode || current[`${prefix}PostalCode`],
      [`${prefix}Country`]: location.country || current[`${prefix}Country`],
      [`${prefix}CountryCode`]:
        location.countryCode || current[`${prefix}CountryCode`],
  ...
  async function calculateManagedQuote() {
    ...
    try {
      ...
      toast.update(toastId, {
        type: "success",
        message: "Service price calculated.",
      });
    } catch (next) {
  ...
  async function submit(event) {
    ...
    if (isShipping && !selectedProvider) {
      toast.warning(
        providerStatus === "loading"
          ? "Available providers are still loading."
          : "Select a service level and provider before payment."
      );
      return;
    }
    if (!terms) {
    ...
    try {
      ...
      const paymentResult = await openServicePayment(
        session,
        id,
        service.title
      );
      toast.update(toastId, {
        type: "success",
        message:
          paymentResult?.booking?.status === "confirmed"
            ? "Payment completed and the provider confirmed your shipment."
            : "Payment completed. Provider confirmation is still pending.",
      });
      navigate(`/services/requests/${id}`, {
        replace: true,
        state: { paymentComplete: true, paymentResult },
      });
    } catch (next) {
      setError(next.message || "Payment could not be completed. Please retry.");
      toast.update(toastId, {
        type: "error",
        message: next.message || "Payment could not be completed.",
      });
    } finally {
      setBusy("");
    }
    ...
    <AppShell>
      <div className="container service-booking-page">
        ...
        <header>
          ...
          <p>
            {isShipping
              ? "Complete the shipment details, request live rates, and select an available carrier service."
              : "Provide the service details, review pricing and pay securely."}
          </p>
          {isShipping && <ProviderStrip keys={service.providers} compact />}
        </header>
        <form
          ref={formRef}
          noValidate
          onSubmit={submit}
          className="service-booking-layout"
        >
          <div>
            {sections.map((section) => (
              <FormSection
                ...
                fields={service.fields.filter(
                  (item) =>
                    (item.step || "Booking details") === section &&
                    isServiceFieldVisible(item, values)
                )}
                values={values}
                errors={fieldErrors}
            ...
            <details className="module-panel service-form-section optional-fields supporting-documents">
              <summary>
                <span>
              ...
              <p>
                Upload invoices, specifications, or other relevant evidence.
              </p>
              <AttachmentUploader
            ...
            {isShipping && (
              <ProviderSelection
                ...
                ready={Boolean(providerFingerprint)}
                selected={selectedProvider}
                disabled={Boolean(requestId)}
                onSearch={() => {
                  if (validate()) void loadProviders();
                }}
                onSelect={(option) => {
          ...
          <aside className="module-panel service-quote-card">
            ...
            {requestId && (
              <p>
                <CheckCircle2 /> Booking saved. Complete payment to confirm with
                the provider.
              </p>
            )}
            <button
              ...
              disabled={
                Boolean(busy) || !terms || (isShipping && !selectedProvider)
              }
            >
              <CreditCard />{" "}
              {busy === "submit"
            ...
            <small>
              Provider booking begins only after Razorpay verifies payment.
            </small>
          </aside>
        </form>
      </div>
...
function FormSection({
  ...
  const render = (field) => (
    <ServiceField
      ...
      disabled={disabled}
      onAddressSelect={(location) => onAddressSelect(field.key, location)}
      countryCode={
        field.key.startsWith("pickup")
          ? values.pickupCountryCode
          : field.key.startsWith("destination")
          ? values.destinationCountryCode
          : ""
      }
  ...
  return (
    <section className="module-panel service-form-section">
      <h2>{title}</h2>
      <div className="form-grid">{required.map(render)}</div>
      {optional.length > 0 && (
        <details className="optional-fields">
          <summary>
            Optional details <small>{optional.length}</small>
          </summary>
          <div className="form-grid">{optional.map(render)}</div>
        </details>
      )}
    </section>
  );
...
function ProviderSelection({
  ...
  const providerKeys = [
    ...new Set([
      ...providerStatuses.map((item) => item.provider),
      ...options.map((item) => item.providerKey),
    ]),
  ];
  return (
    <section className="module-panel service-form-section provider-results">
      <header>
        <div>
        ...
        {result?.expiresAt && (
          <small>
            Rates valid until {new Date(result.expiresAt).toLocaleTimeString()}
          </small>
        )}
      </header>
      {status !== "loading" && (
      ...
      {status === "loading" && (
        <div className="provider-load-summary is-loading">
          <LoaderCircle />
          <span>
            <b>Finding available shipping services...</b>
            <small>Checking each configured provider securely.</small>
          </span>
        </div>
      )}
            ...
            <div key={item.provider} className={`is-${item.status}`}>
              ...
              <span>
                {item.status === "checking" ? (
                  <>
                    <LoaderCircle /> Checking...
                  </>
                ) : item.status === "connected" ? (
                  <>
                    <CheckCircle2 /> Rates available
                  </>
                ) : item.status === "no_rates" ? (
                  "No services for this route"
                ) : (
      ...
      {status === "loaded" && options.length === 0 && (
        <div className="empty-results shipping-empty">
          <PackageCheck />
          <h3>No shipping services available</h3>
          <p>
            We couldn't find an available shipping service for this shipment.
            Check the pickup address, destination and package details and try
            again.
          </p>
        </div>
      )}
      <p className="provider-rate-intro">
        {status === "idle"
          ? "Complete the shipment details, then request live rates."
          : status === "loading"
          ? "Finding available shipping services..."
          : status === "error"
          ? "Shipping providers are currently unavailable. Review the details and retry."
          : options.length
          ? "Choose a service and rate to continue."
          : "No shipping services are available for these details."}
      </p>
      {options.length > 0 && (
        <div
          className="shipping-rate-cards"
          aria-label="Available shipping services"
        >
          {providerKeys.flatMap((providerKey) =>
            options
              .filter((option) => option.providerKey === providerKey)
              .map((option) => (
                <ProviderRateCard
                  key={option.quoteId}
                  option={option}
                  selected={selected?.quoteId === option.quoteId}
                  disabled={disabled}
      ...
      {status === "loaded" &&
        providerStatuses.some((item) => item.status === "failed") && (
          <p className="provider-partial-note">
            Some configured providers are currently unavailable. Available live
    ...
    >
      <header>
        ...
        <div className="provider-badges">
          ...
          {option.bestPrice && (
            <em>
              <BadgeCheck /> Best price
            </em>
          )}
        </div>
      </header>
      <div className="shipping-rate-service">
        <small>
      ...
      <div className="shipping-rate-route">
        ...
        <span>
          <small>From</small>
          <b>{locationLabel(option.origin)}</b>
        </span>
        <i>→</i>
        <span>
          <small>To</small>
          <b>{locationLabel(option.destination)}</b>
        </span>
      </div>
      <div className="shipping-rate-facts">
        <span>
          <small>Estimated delivery</small>
          <b>{deliveryText(option)}</b>
        </span>
...
function ProviderLoadSummary({ status, ready }) {
  return (
    <div className={`provider-load-summary is-${status}`}>
      {status === "loading" ? (
        <LoaderCircle />
      ) : status === "loaded" ? (
        <CheckCircle2 />
  ...
  return (
    <div className="quote-breakdown">
      ...
      <span>
        Platform fee{" "}
        <b>
          <Money value={pricing.platformFee} currency={pricing.currency} />
        </b>
      </span>
      {pricing.gstAmount > 0 && (
        <span>
          GST ({pricing.gstRate}%){" "}
          <b>
            <Money value={pricing.gstAmount} currency={pricing.currency} />
          </b>
        </span>
      )}
      <strong>
        Total{" "}
        <b>
          <Money value={pricing.totalPayable} currency={pricing.currency} />
        </b>
      </strong>
    </div>
  );
...
function deliveryText(option) {
  return (
    ...
    (option.estimatedDeliveryAt &&
      new Date(option.estimatedDeliveryAt).toLocaleDateString()) ||
...
function publicFormLocation(value = {}) {
  return {
    city: value.city,
    state: value.state,
...
function mergeProviderResult(current = {}, result = {}, providerKey) {
  const providers = applyClientRateBadges([
    ...(current.providers || []).filter(
      (option) => option.providerKey !== providerKey
    ),
    ...(result.providers || []),
  ]);
  const incomingStatus = result.providerStatuses?.find(
    (item) => item.provider === providerKey
  ) || {
    provider: providerKey,
    status: providers.some((option) => option.providerKey === providerKey)
      ? "connected"
      : "no_rates",
  };
  return {
    ...
    providerStatuses: (current.providerStatuses || []).map((item) =>
      item.provider === providerKey ? incomingStatus : item
    ),
...
function applyClientRateBadges(options) {
  ...
  const etaValues = options.map((option) =>
    option.estimatedDeliveryAt
      ? new Date(option.estimatedDeliveryAt).getTime()
  ...
  return options.map((option) => ({
    ...
    fastest:
      ...
      option.estimatedDeliveryAt &&
      new Date(option.estimatedDeliveryAt).getTime() === fastest,
    recommended:
      Number(option.price) === lowest &&
  ...
  return {
    ...
    providerStatuses: (current.providerStatuses || []).map((item) =>
      item.provider === providerKey
        ? { provider: providerKey, status: "failed", code }
        : item
    ),
  };
}
...
function providerFieldErrors(errors = {}) {
  const fieldMap = {
    "pickup.contactName": "pickupContactName",
    "pickup.phone": "pickupPhone",
    "pickup.email": "pickupEmail",
    "pickup.line1": "pickupLine1",
    "pickup.city": "pickupCity",
    "pickup.state": "pickupState",
    "pickup.postalCode": "pickupPostalCode",
    "pickup.country": "pickupCountry",
    "pickup.countryCode": "pickupCountryCode",
    "destination.contactName": "destinationContactName",
    "destination.phone": "destinationPhone",
    "destination.email": "destinationEmail",
    "destination.line1": "destinationLine1",
    "destination.city": "destinationCity",
    "destination.state": "destinationState",
    "destination.postalCode": "destinationPostalCode",
    "destination.country": "destinationCountry",
    "destination.countryCode": "destinationCountryCode",
  };
  return Object.fromEntries(
    Object.entries(errors).map(([key, message]) => [
      fieldMap[key] || key.replace(/^shipment\./, ""),
      message,
    ])
  );
}
