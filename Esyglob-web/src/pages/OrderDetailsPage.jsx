...
import { addProductionUpdate, buyerOrderAction, fetchOrder, initiatePayment, markOrderReadyForShipment, updateOrder, verifyPayment } from '../api/trade'
...
async function pay() {
  ...
  const order=query.data||{},product=typeof order.productId==='object'?order.productId:order.products?.[0]||{},timeline=order.timeline||order.statusHistory||[],workflow=order.workflowSnapshot||{},allowed=(workflow.allowedNextStages||[]).filter(status=>!['ready_to_ship','preparing_shipment','pickup_scheduled','picked_up','warehouse_processing','in_transit','custom_clearance','out_for_delivery','shipped','delivered','returned'].includes(status))
  ...
  {order.paymentStatus==='paid'&&<section className="agreement-progress"><Truck/><div><b>{order.trackingNumber?'Shipment tracking is available':'Shipment awaiting readiness'}</b><p>{order.trackingNumber?'Follow the live carrier journey and delivery milestones.':sellerView?'Confirm the package is ready to initiate carrier booking.':'The seller is preparing your shipment. Tracking will appear after carrier booking.'}</p></div>{order.trackingNumber?<Link className="button button--secondary" to={`/orders/${orderId}/tracking`}>Track shipment</Link>:sellerView&&<button className="button button--primary" disabled={busy} onClick={()=>perform(()=>markOrderReadyForShipment(orderId),'Shipment marked ready and carrier booking started.')}>Mark ready for shipment</button>}</section>}
