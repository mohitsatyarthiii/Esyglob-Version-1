import { ArrowLeft, BadgeCheck, Building2, Camera, Check, CheckCheck, FileText, Image, MapPin, MessageSquare, MoreVertical, Paperclip, Plus, Send, Store, UserRound, X } from 'lucide-react'
...
export default function ChatPage() {
  ...
  const buyerProfile = state.data?.buyerProfile || (!isBuyer && typeof chat.buyerId === 'object' ? chat.buyerId : null)
  ...
  return <AppShell><section className="chat-screen container" aria-label={`Conversation with ${participant}`}>
    <header className="chat-header">
      ...
      {isBuyer && sellerProfile?._id && <Link className="chat-supplier-link" to={`/sellers/${sellerProfile._id}`}><Store /> Manufacturer profile</Link>}
