...
export default function AddressesPage() {
  ...
  async function save(event) { event.preventDefault(); setBusy(true); setError(''); try { const id = resolveId(editing); if (id) await updateAddress(id, form); else await createAddress(form); setEditing(null); toast.success(id ? 'Address updated.' : 'Address saved.'); await query.reload() } catch (next) { setError(next.message) } finally { setBusy(false) } }
  async function remove(item) { if (!await confirm({ title: 'Delete saved address?', message: 'This address will be removed from checkout and your address book.', confirmLabel: 'Delete address' })) return; await deleteAddress(resolveId(item)); toast.success('Address deleted.'); query.reload() }
