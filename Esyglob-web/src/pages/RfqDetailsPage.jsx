...
function LegacyQuotationForm({ rfq, onClose, onSuccess }) {
  const [attachments, setAttachments] = useState([])
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState('')
  const [form, setForm] = useState({ suppliedQuantity: rfq.quantity || 1, minimumOrderQuantity: rfq.minimumOrderQuantity || 1, unitPrice: '', currency: rfq.currency || 'INR', leadTime: '15', leadTimeUnit: 'days', productionTime: '15', productionTimeUnit: 'days', packaging: '', samplePrice: '', taxRate: '', shippingTerms: '', paymentTerms: '30% advance, balance before dispatch', advanceRequired: 30, incoterms: rfq.incoterms || 'FOB', shippingCost: 0, shippingEstimate: '', expiryDate: '', specifications: '', sellerMessage: '' })
...
function QuotationForm({ rfq, onClose, onSuccess }) {
  ...
  const [form, setForm] = useState({
    ...
    suppliedQuantity: rfq.quantity || 1,
    leadTime: '15',
    leadTimeUnit: 'days',
    shippingTerms: '',
  ...
  const total = useMemo(
    ...
    [form.suppliedQuantity, form.unitPrice],
  )
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><form className="quotation-modal quotation-modal--compact" onMouseDown={(event) => event.stopPropagation()} onSubmit={submit}>
    ...
    <div className="form-grid form-grid--3">
      ...
      <Field label="Available quantity" required><input type="number" min="1" value={form.suppliedQuantity} onChange={(event) => update('suppliedQuantity', event.target.value)} required /></Field>
    </div>
    <div className="form-grid">
      <Field label="Lead time" required><div className="compound-input"><input type="number" min="1" value={form.leadTime} onChange={(event) => update('leadTime', event.target.value)} required /><select value={form.leadTimeUnit} onChange={(event) => update('leadTimeUnit', event.target.value)}><option>days</option><option>weeks</option></select></div></Field>
      <Field label="Valid until"><input type="date" value={form.expiryDate} onChange={(event) => update('expiryDate', event.target.value)} /></Field>
