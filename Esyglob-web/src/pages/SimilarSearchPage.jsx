...
import { fetchProductDetails, fetchRelatedProducts, searchByImage } from '../api/marketplace'
...
export default function SimilarSearchPage() {
  ...
  const query = useAsyncData(useCallback(async () => {
    ...
    const [products, visual] = await Promise.all([
      ...
      image ? searchByImage(image).catch(() => ({})) : Promise.resolve({}),
    ])
    return { ...visual, products }
  }, [productId]))
