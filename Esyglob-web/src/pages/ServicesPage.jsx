...
import { fetchServiceRequests, isServiceAvailable, servicesForRole } from '../api/services'
...
export default function ServicesPage() {
  ...
  const [role, setRole] = useState(roles.includes('seller') && user?.primaryRole === 'seller' ? 'seller' : 'buyer')
  const [category, setCategory] = useState('All')
  const [search, setSearch] = useState('')
  const activity = useAsyncData(useCallback(
  ...
  const visible = useMemo(
    () => catalog.filter(item => (category === 'All' || item.category === category)
  ...
  const comingSoonServices = visible.filter(item => !isServiceAvailable(item))
  const active = activity.data?.filter(item => !['completed', 'cancelled'].includes(item.status)).length || 0

  return <AppShell><div className="container services-page">
    <PageHead eyebrow="End-to-end trade operations" title="Trade services" description="Book, pay for and track professional support across logistics, finance, quality, verification and compliance." />
    ...
    {authenticated && <section className="service-overview"><div><span>Available services</span><b>{catalog.filter(isServiceAvailable).length}</b></div><div><span>Active requests</span><b>{active}</b></div><div><span>Total bookings</span><b>{activity.data?.length || 0}</b></div><Link to="/services/requests">Booking history <ArrowRight /></Link></section>}
    ...
    {!!availableServices.length && <><div className="service-section-heading"><span>Available services</span><h2>Ready when you are</h2><p>Start a secure request, receive supported pricing, and track progress in one place.</p></div><div className="service-catalog">{availableServices.map(item => {
      ...
      return <article className={`service-accent service-accent--${item.key}`} key={item.key}>
        ...
        <footer><div><small>{item.key === 'shipping' ? 'Pricing' : 'Starting at'}</small><b>{item.startingPriceAmount !== null ? <><span>From </span><Money value={item.startingPriceAmount} currency={item.startingPriceCurrency} /></> : item.startingPrice}</b></div><Link to={`/services/${item.key}`}>{item.key === 'shipping' ? 'View rates' : 'View service'} <ArrowRight /></Link></footer>
      </article>
    })}</div></>}
    {!!comingSoonServices.length && <section className="coming-soon-services"><div className="service-section-heading"><span>Coming Soon</span><h2>More ways to trade with confidence</h2><p>These services are being prepared intentionally and are not yet open for booking.</p></div><div className="service-catalog service-catalog--soon">{comingSoonServices.map(item => { const Icon = icons[item.category] || Boxes; return <article className={`service-accent service-accent--${item.key}`} key={item.key}><span className="coming-soon-badge">Coming Soon</span><header className="service-card-heading"><div className="service-card-icon"><Icon /></div><span>{item.category}</span></header><h2>{item.title}</h2><p>{item.description}</p><footer><div><small>Availability</small><b>In preparation</b></div><Link to={`/services/${item.key}`}>Learn more <ArrowRight /></Link></footer></article> })}</div></section>}
