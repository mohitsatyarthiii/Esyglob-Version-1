...
export default function FinalQuotationPanel({ quotationId }) {
  ...
  return <section className="container agreement-lifecycle-section" aria-labelledby="final-quotation-title">
    ...
    <header><div><span className="eyebrow"><ShieldCheck /> Official commercial document</span><h2 id="final-quotation-title">Final Quotation</h2><p>The Seller signs first. The Buyer then explicitly confirms and signs the locked commercial record before an order can be created.</p></div><span className={`agreement-state agreement-state--${document?.status || quotation.finalQuotation?.status || 'preparation'}`}>{label(document?.status || quotation.finalQuotation?.status || 'Seller preparation')}</span></header>
...
function FinalPreparation({ quotation, onComplete, setError }) {
  ...
  const [form] = useState({ suppliedQuantity: quotation.suppliedQuantity || '', minimumOrderQuantity: quotation.minimumOrderQuantity || 1, unitPrice: quotation.unitPrice || '', leadTime: quotation.leadTime || '', shippingTerms: quotation.shippingTerms || '', paymentTerms: quotation.paymentTerms || '', expiryDate: quotation.expiryDate ? String(quotation.expiryDate).slice(0, 10) : '', notes: quotation.notes || quotation.sellerMessage || '' })
  ...
  async function submit(event) {
    ...
    try {
      await updateQuotation(id(quotation), { action: 'confirm', expectedNegotiationVersion: Number(quotation.negotiationVersion || 0), idempotencyKey: actionToken(), attachments, reason: 'Seller prepared the Final Quotation from the accepted terms' })
  ...
  return <form className="agreement-preparation" onSubmit={submit}>
    ...
    <p className="form-help">The accepted price, quantity, delivery, shipping, payment, and validity terms are locked. Reopen negotiation if either party needs a commercial change.</p>
...
function FinalPreview({ quotation, form, total }) {
  ...
  return <section className="live-agreement-preview">
    ...
    <footer><span>Seller information verified</span><span>Two-party signature audit</span></footer>
  </section>
}
...
function FinalDocument({ data, quotation, document, versions, user, authStatus, onComplete, setError }) {
  ...
  async function sign() {
    ...
    try {
      ...
      setSignOpen(false)
      await onComplete(actorRole === 'seller' ? 'Seller signature recorded. The Buyer can now review and sign.' : 'Final Quotation fully signed and locked. Place Order is now available.')
    } catch (next) { setError(next.message) } finally { setBusy(false) }
  }
  async function requestChanges() {
    ...
    setBusy(true); setError('')
    try {
      await updateQuotation(id(quotation), { action: 'request_revision', expectedNegotiationVersion: Number(quotation.negotiationVersion || 0), idempotencyKey: actionToken(), reason: changeReason.trim(), buyerMessage: changeReason.trim(), attachments: changeFiles })
  ...
  return <div className="agreement-document">
    ...
    {signed && <div className="agreement-active-banner"><CheckCircle2 /><div><b>Final Quotation fully signed and permanently locked</b><p>Both signatures are embedded in the official PDF. The order will inherit these locked commercial terms.</p></div>{actorRole === 'buyer' ? <button className="button button--primary" disabled={busy} onClick={startCheckout}><PackageCheck /> {busy ? 'Opening checkout…' : 'Place Order'}</button> : data.order?._id ? <Link className="button button--primary" to={`/orders/${id(data.order)}?role=seller`}><PackageCheck /> Open Order</Link> : <span className="agreement-state agreement-state--awaiting_buyer_signature">Waiting for Buyer order</span>}</div>}
