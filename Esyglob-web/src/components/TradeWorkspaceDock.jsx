...
export default function TradeWorkspaceDock() {
  ...
  const context = useMemo(() => {
    const match = pathname.match(/^\/(rfqs|quotations|orders)\/([^/]+)$/)
    if (!match || ['new', 'compare'].includes(match[2])) return null
    return { entityType: match[1] === 'rfqs' ? 'rfq' : match[1] === 'orders' ? 'order' : 'quotation', entityId: match[2] }
  }, [pathname])
