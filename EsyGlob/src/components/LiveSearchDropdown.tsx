...
function getProductName(product: any): string {
  return  product?.name ?? product?.title ?? product?.productName ?? product?.productTitle;
}

function getProductImage(product: any): string | undefined {
  return product?.image || product?.images?.[0] || product?.product?.image;
}

function getSupplierName(supplier: any): string {
  return supplier?.companyName || supplier?.businessName || supplier?.displayName || 'Supplier';
}

function getSupplierLogo(supplier: any): string | undefined {
  return supplier?.logo || supplier?.companyLogo || supplier?.logoUrl;
}
...
function isSupplierVerified(supplier: any): boolean {
  return supplier?.isVerified === true || supplier?.verificationStatus === 'verified';
}
  ...
  const handleSelectProduct = useCallback(
    (product: any) => {
      ...
      navigation.navigate('ProductDetails', {
        productId: product._id || getId(product),
      });
    },
    [navigation, onClose],
  );
  ...
  const handleSelectSupplier = useCallback(
    (supplier: any) => {
      ...
      navigation.navigate('SellerDetails', {
        sellerId: supplier._id || getStableKey(supplier),
