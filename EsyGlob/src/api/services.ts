...
function tagServiceSource(items: ServiceRequest[], source: string): ServiceRequest[] {
  return items.map(item => ({ ...item, _serviceSource: source }));
}
...
export async function cancelServiceActivity(request: ServiceRequest) {
  ...
  throw new Error('Cancellation is not available for this service after submission. Contact support for assistance.');
}