...
const BUYER_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: 'crown-outline', label: 'Subscription', color: P.primary },
...
function applyBadges(
  sections: typeof BUYER_SECTIONS,
  badges: Record<string, string>,
): DashboardSection[] {
  return sections.map(s => ({
    ...s,
    items: s.items.map(i => ({ ...i, badge: badges[i.label] })),
  }));
}
    ...
    <View>
      ...
      >
        ...
        {loading ? (
          ...
          <Icon name="chevron-down" size={14} color={P.muted} />
        )}
      </Pressable>
        ...
        <View style={styles.modalWrap}>
          ...
          <View style={styles.modalSheet}>
            ...
            <View style={styles.modalHead}>
              ...
              >
                <Icon name="close" size={20} color={P.ink} />
              </Pressable>
            </View>
            <View style={styles.searchWrap}>
              ...
              {q ? (
                <Pressable onPress={() => setQ('')}>
                  <Icon name="close-circle" size={14} color={P.muted} />
                </Pressable>
              ) : null}
            </View>
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                return (
                  <Pressable
                    style={[styles.curItem, active && styles.curItemActive]}
                    onPress={() => {
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      >
                        {item.code}{' '}
                        <Text style={styles.curItemSym}>{item.symbol}</Text>
                      </Text>
                      <Text style={styles.curItemName}>
                        {item.name} • {item.country}
                      </Text>
                    </View>
                    {active && (
                      <Icon name="check-circle" size={20} color={P.primary} />
                    )}
                  </Pressable>
                );
              }}
...
function Benefit({
  ...
  return (
    <View style={styles.benRow}>
      <View style={[styles.benIcon, { backgroundColor: `${color}15` }]}>
        <Icon name={icon} size={16} color={color} />
      </View>
      <View>
        <Text style={styles.benTitle}>{title}</Text>
        <Text style={styles.benSub}>{sub}</Text>
      </View>
    </View>
  );
}
...
function AccountScreen() {
  ...
  const rfqQ = useQuery({
    queryKey: ['acc-rfq', role],
    queryFn: () => fetchRFQs({ scope: role, limit: 1 }),
    enabled: status === 'authenticated',
    ...QO,
    select: (d: any) => ({
      count: d?.pagination?.total ?? d?.rfqs?.length ?? 0,
    }),
  });
  const chatQ = useQuery({
    queryKey: ['acc-chat', role],
    queryFn: () => fetchChats(role),
    enabled: status === 'authenticated',
    ...QO,
    select: (d: any) => ({ count: Array.isArray(d) ? d.length : 0 }),
  });
  const verificationQ = useQuery({
    queryKey: ['seller-onboarding'],
    queryFn: fetchSellerOnboarding,
    enabled: status === 'authenticated' && role === 'seller',
    staleTime: 20_000,
  });
  ...
  const sellerSec = useMemo(
    () => applyBadges(SELLER_SECTIONS, badges),
    [badges],
  );

  useEffect(() => {
    if (status === 'authenticated' && prevMode) {
      setAuthMode(null);
      setWelcome(true);
      setPrevMode(null);
    }
  ...
  const refresh = () => {
    qc.invalidateQueries({ queryKey: ['acc-notif'] });
      ...
      >
        ...
        <View style={styles.guestCard}>
          <View style={styles.guestAvatar}>
            <Icon name="account-outline" size={28} color={P.muted} />
          </View>
          <Text style={styles.guestTitle}>Welcome to EsyGlob</Text>
          <Text style={styles.guestSub}>
            Sign in to manage orders, RFQs, and supplier messages.
          </Text>
          <View style={styles.guestBtns}>
            <Pressable
              onPress={() => setAuthMode('login')}
              style={styles.guestBtnPri}
            >
              <Text style={styles.guestBtnPriText}>Login</Text>
            </Pressable>
            <Pressable
              onPress={() => setAuthMode('signup')}
              style={styles.guestBtnSec}
            >
              <Text style={styles.guestBtnSecText}>Sign up</Text>
            </Pressable>
          </View>
        </View>
        <View style={styles.guestCard}>
          ...
          <Benefit
            ...
            sub="Build trust with partners"
            color={P.emerald}
          />
          <Benefit
            icon="message-text-outline"
            title="Buyer-seller messaging"
            sub="Real-time communication"
            color={P.violet}
          />
          <Benefit
            icon="heart-outline"
            title="Wishlist & history"
            sub="Save and track products"
            color={P.rose}
          />
        </View>
  ...
  {role === 'seller' ? 'Esyglob Seller Account' : 'Esyglob Buyer Account'}
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl
            refreshing={
              notifQ.isFetching || rfqQ.isFetching || chatQ.isFetching
            }
            onRefresh={refresh}
            tintColor={P.primary}
          />
      ...
      >
        ...
        >
          <RemoteImage
            ...
            fallback={
              <View style={styles.profFall}>
                <Text style={styles.profFallText}>
        ...
        {role === 'seller' && verificationQ.data
          ? (() => {
              const data = verificationQ.data as any;
              const summary =
                data.verificationCenter?.summary ??
                data.verificationCenter ??
                data.completion ??
                {};
              const completion = Number(
                summary.completionPercentage ?? summary.percentage ?? 0,
              );
              const trust = Number(
                summary.overallTrustScore ?? data.seller?.trustScore ?? 0,
              );
              const level =
                summary.currentLevel ??
                summary.verificationLevel ??
                data.seller?.verificationLevel ??
                'Starter';
              const complete =
                completion >= 100 ||
                String(data.seller?.verificationStatus).toLowerCase() ===
                  'verified';
              return (
                <Pressable
                  ...
                  style={[
                    ...
                    complete && styles.verificationComplete,
                  ]}
                >
                  <View
                    style={[
                      styles.verificationIcon,
                      complete && styles.verificationIconComplete,
                    ]}
                  >
                    <Icon
                      name={complete ? 'check-decagram' : 'shield-star-outline'}
                      size={25}
                      color={complete ? '#FFF' : P.primary}
                    />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text
                      style={[
                        ...
                        complete && styles.verificationTitleComplete,
                      ]}
                    >
                      {complete
                    ...
                    <Text
                      style={[
                        ...
                        complete && styles.verificationMetaComplete,
                      ]}
                    >
                      Trust {trust}/100 · Level {String(level)}
                      {!complete
                        ? ` · ${Math.max(0, 100 - completion)}% remaining`
                        : ''}
                    </Text>
                    {!complete ? (
                ...
                >
                  ...
                  >
                    ...
                    {item.badge && (
                      ...
                      >
                        <Text style={styles.secBadgeText}>{item.badge}</Text>
                      </View>
                    )}
                  </View>
                  <Text style={styles.secLabel} numberOfLines={1}>
                    {item.label}
                  </Text>
                </Pressable>
              ))}
        ...
        <View style={styles.secCard}>
          <Pressable
            ...
            style={styles.actRow}
          >
            <Icon name="cog-outline" size={16} color={P.primary} />
            <Text style={styles.actText}>Settings</Text>
            <Icon name="chevron-right" size={14} color={P.faint} />
          </Pressable>
          <Pressable
            onPress={() => nav.navigate('Security')}
            style={styles.actRow}
          >
            <Icon name="shield-lock-outline" size={16} color={P.emerald} />
            <Text style={styles.actText}>Security & Privacy</Text>
            <Icon name="chevron-right" size={14} color={P.faint} />
          </Pressable>
          <Pressable
            onPress={() => nav.navigate('HelpSupport')}
            style={[styles.actRow, { borderBottomWidth: 0 }]}
          >
            <Icon name="lifebuoy" size={16} color={P.red} />
            <Text style={styles.actText}>Help & Support</Text>
            <Icon name="chevron-right" size={14} color={P.faint} />
          </Pressable>
        </View>
