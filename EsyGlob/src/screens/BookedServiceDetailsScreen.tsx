...
import { initiateServicePayment, servicePaymentFailureStatus, updateServicePaymentStatus, verifyServicePayment } from '../api/services';
  ...
  return (
    <View style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.detailContent} showsVerticalScrollIndicator={false}>
        ...
        <Section title="Provider and tracking"><View style={styles.infoRow}><Text style={styles.infoKey}>Provider</Text><Text style={styles.infoValue}>{String(request.provider?.name ?? request.assignedTeam ?? request.assignedProvider ?? request.assignedTo ?? request.providerName ?? 'Assignment pending')}</Text></View><View style={styles.infoRow}><Text style={styles.infoKey}>Provider service</Text><Text style={styles.infoValue}>{String(request.provider?.serviceName ?? 'Confirmation pending')}</Text></View><View style={styles.infoRow}><Text style={styles.infoKey}>Tracking number</Text><Text style={styles.infoValue}>{String(request.provider?.trackingNumber ?? 'Assigned after booking')}</Text></View><View style={styles.infoRow}><Text style={styles.infoKey}>ETA</Text><Text style={styles.infoValue}>{request.provider?.eta ? new Date(request.provider.eta).toLocaleString() : 'Awaiting provider ETA'}</Text></View></Section>
